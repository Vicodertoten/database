...
def test_subset_builder_is_deterministic_and_respects_limits(tmp_path: Path) -> None:
    snapshot_root = tmp_path / "data/raw/inaturalist"
    _build_source_snapshot(snapshot_root, "source")

    audit_a = build_controlled_inat_snapshot_subset(
        snapshot_id="source",
        output_snapshot_id="subset-a",
        max_media_count=3,
        max_media_per_taxon=2,
        snapshot_root=snapshot_root,
    )
    audit_b = build_controlled_inat_snapshot_subset(
        snapshot_id="source",
        output_snapshot_id="subset-b",
        max_media_count=3,
        max_media_per_taxon=2,
        snapshot_root=snapshot_root,
    )

    assert audit_a["selected_media_count"] == 3
    assert audit_a["selected_media_count_by_taxon"] == audit_b["selected_media_count_by_taxon"]
    assert max(audit_a["selected_media_count_by_taxon"].values()) <= 2


def test_subset_builder_writes_manifest_and_leaves_source_untouched(tmp_path: Path) -> None:
    snapshot_root = tmp_path / "data/raw/inaturalist"
    _build_source_snapshot(snapshot_root, "source")
    source_manifest_before = (snapshot_root / "source" / "manifest.json").read_text(
        encoding="utf-8"
    )

    build_controlled_inat_snapshot_subset(
        snapshot_id="source",
        output_snapshot_id="subset",
        max_media_count=2,
        snapshot_root=snapshot_root,
    )

    subset_manifest = json.loads(
        (snapshot_root / "subset" / "manifest.json").read_text(encoding="utf-8")
    )
    assert subset_manifest["snapshot_id"] == "subset"
    assert len(subset_manifest["media_downloads"]) == 2
    assert (snapshot_root / "subset" / "subset_audit.json").exists()
    assert (snapshot_root / "source" / "manifest.json").read_text(
        encoding="utf-8"
