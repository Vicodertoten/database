...
from database_core.versioning import (
    ...
    PACK_MATERIALIZATION_V2_VERSION,
...
class PostgresPackStore:
    ...
    def compile_pack_v2(
        ...
        *,
        ...
        policy = (
            distractor_policy
            if isinstance(distractor_policy, DistractorPolicy)
            else DistractorPolicy(**(distractor_policy or {}))
        )
        context = self.compute_pack_compilation_context_v2(
            connection,
            pack_id=pack_id,
            revision=revision,
            question_count_requested=max(question_count, MIN_PACK_TOTAL_QUESTIONS),
        ...
        if not context["compilable"]:
            ...
            raise ValueError(
                "Pack is not compilable for v2 build persistence "
                f"(reason_code={context['reason_code']}, deficits={deficits})"
            )

        build_id = f"packbuild:{pack_id}:{context['revision']}:v2:{uuid4().hex[:8]}"
        built_at = datetime.now(UTC)
        built_questions = context["questions"][:question_count]
        build = CompiledPackBuildV2(
            build_id=build_id,
            pack_id=pack_id,
            revision=int(context["revision"]),
            built_at=built_at,
        ...
        payload = {
            "schema_version": SCHEMA_VERSION_LABEL,
            "pack_compiled_version": COMPILED_PACK_V2_VERSION,
            **build.model_dump(mode="json"),
        }
        validate_compiled_pack(payload)
        connection.execute(
    ...
    def materialize_pack_v2(
        ...
        *,
        ...
        if connection is None:
            with self._connect() as owned_connection:
                return self.materialize_pack_v2(
                    pack_id=pack_id,
                    revision=revision,
                    question_count=question_count,
                    purpose=purpose,
        ...
        if latest_build is None:
            raise ValueError(
                "No v2 compiled build found for materialization. "
                "Run `pack compile --contract-version v2` first for this revision."
            )
        ...
        materialization = MaterializedPackV2(
            materialization_id=(
                f"packmat:{pack_id}:{revision_value}:{purpose_value}:v2:{uuid4().hex[:8]}"
            ),
            pack_id=pack_id,
            revision=revision_value,
            source_build_id=str(latest_build["build_id"]),
            ...
            expires_at=expires_at,
            question_count=len(selected_questions),
            questions=[CompiledPackQuestionV2(**item) for item in selected_questions],
        )
        payload = {
            "schema_version": SCHEMA_VERSION_LABEL,
            "pack_materialization_version": PACK_MATERIALIZATION_V2_VERSION,
            **materialization.model_dump(mode="json"),
        }
        validate_pack_materialization(payload)
        connection.execute(
    ...
    def compute_pack_compilation_context_v2(
        self,
        connection: psycopg.Connection,
        *,
        pack_id: str,
        ...
        external_candidates_by_target = self._map_inat_similarity_candidates(
            connection,
            canonical_taxon_ids=requested_taxa,
            distractor_policy=policy,
        )

        questions = self._build_compiled_questions_v2(
            items_per_taxon=items_per_taxon,
            requested_taxa=requested_taxa,
            difficulty_policy=str(parameters.difficulty_policy),
            question_count_requested=question_count_requested,
            external_candidates_by_target=external_candidates_by_target,
            distractor_policy=policy,
        )
    ...
    def _fetch_latest_compiled_payload(
        ...
        *,
        pack_id: str,
        revision: int,
        pack_compiled_version: str | None = None,
    ) -> dict[str, object] | None:
        version_clause = "AND pack_compiled_version = %s" if pack_compiled_version else ""
        params: list[object] = [pack_id, revision]
        if pack_compiled_version:
            params.append(pack_compiled_version)
        row = connection.execute(
            f"""
            SELECT payload_json
            FROM compiled_pack_builds
    ...
    def _build_compiled_questions_v2(
        self,
        *,
        items_per_taxon: dict[str, list[dict[str, object]]],
        requested_taxa: Sequence[str],
        difficulty_policy: str,
        question_count_requested: int,
        external_candidates_by_target: dict[str, list[dict[str, object]]],
        distractor_policy: DistractorPolicy,
    ) -> list[CompiledPackQuestionV2]:
        sorted_taxon_ids = list(dict.fromkeys(requested_taxa))
        for canonical_taxon_id in sorted_taxon_ids:
            items_per_taxon[canonical_taxon_id].sort(
                key=lambda row: self._distractor_item_sort_key(
        ...
        used_distractor_taxa: set[str] = set()
        for target_row in target_rows:
            target_taxon = str(target_row["canonical_taxon_id"])
            target_label = self._row_taxon_label(target_row)
            target_similar_taxon_ids = {
                taxon_id
                for taxon_id in self._normalize_similar_taxon_ids(target_row)
                if taxon_id != target_taxon
            ...
            for canonical_taxon_id in sorted_taxon_ids:
                if canonical_taxon_id == target_taxon or not items_per_taxon[canonical_taxon_id]:
                    continue
                row = items_per_taxon[canonical_taxon_id][0]
                reason_codes = ["diversity_fallback"]
                score = 0.10
                if canonical_taxon_id in target_similar_taxon_ids:
                    reason_codes = ["internal_similarity"]
                    score += 0.25
                candidates.append(
                    {
                        "canonical_taxon_id": canonical_taxon_id,
                        "taxon_label": self._row_taxon_label(row),
                        "playable_item_id": str(row["playable_item_id"]),
                        "source": "playable_corpus.v1",
                        "score": score,
                        "reason_codes": reason_codes,
                        "referenced_only": False,
                        "sort_key": self._distractor_item_sort_key(
                            difficulty_policy=difficulty_policy,
                            row=row,
                        ),
                    }
            ...
            for candidate in external_candidates_by_target.get(target_taxon, []):
                candidate_taxon_id = str(candidate["canonical_taxon_id"])
                if candidate_taxon_id == target_taxon:
                    continue
                if (
                    candidate.get("out_of_pack")
                    and not distractor_policy.allow_out_of_pack_distractors
                ):
                    continue
                if (
                    candidate.get("referenced_only")
                    and not distractor_policy.allow_referenced_only_distractors
                ):
                    continue
                candidates.append(candidate)

            selected_candidates: list[dict[str, object]] = []
            seen_taxa = {target_taxon}
            referenced_only_count = 0
            for candidate in sorted(
                candidates,
                key=lambda item: (
                    -float(item.get("score") or 0.0),
                    str(item.get("canonical_taxon_id")) in used_distractor_taxa,
                    item.get("sort_key", (99, 99, 99, "", "")),
                    str(item.get("canonical_taxon_id")),
                ),
            ):
                candidate_taxon_id = str(candidate["canonical_taxon_id"])
                if candidate_taxon_id in seen_taxa:
                    continue
                if bool(candidate.get("referenced_only")):
                    if (
                        referenced_only_count
                        >= distractor_policy.max_referenced_only_distractors_per_question
                    ):
                        continue
            ...
            option_payloads = [
                QuestionOption(
                    option_id=f"q{len(questions) + 1}:opt:target",
                    canonical_taxon_id=target_taxon,
                    taxon_label=target_label,
                    is_correct=True,
                    playable_item_id=str(target_row["playable_item_id"]),
                    source="playable_corpus.v1",
                    score=1.0,
                    reason_codes=[],
                    referenced_only=False,
            ...
            for index, candidate in enumerate(selected_candidates, start=1):
                ...
                if candidate.get("out_of_pack") and "out_of_pack" not in reason_codes:
                    reason_codes.append("out_of_pack")
                if candidate.get("referenced_only") and "referenced_only" not in reason_codes:
                    reason_codes.append("referenced_only")
                option_payloads.append(
                    QuestionOption(
                        option_id=f"q{len(questions) + 1}:opt:d{index}",
                        canonical_taxon_id=str(candidate["canonical_taxon_id"]),
                        taxon_label=str(candidate["taxon_label"]),
                        is_correct=False,
                        playable_item_id=(
                            str(candidate["playable_item_id"])
                            if candidate.get("playable_item_id") is not None
                            else None
                        ),
                        source=str(candidate["source"]),
                        score=float(candidate.get("score") or 0.0),
                        reason_codes=reason_codes,
                        referenced_only=bool(candidate.get("referenced_only")),
                    )
                )
                used_distractor_taxa.add(str(candidate["canonical_taxon_id"]))
    ...
    def _map_inat_similarity_candidates(
        self,
        connection: psycopg.Connection,
        *,
        canonical_taxon_ids: Sequence[str],
        ...
        metadata_by_taxon_id = self._fetch_canonical_taxon_metadata(connection)
        inat_external_id_to_canonical_taxon_id: dict[str, str] = {}
        for canonical_taxon_id, metadata in metadata_by_taxon_id.items():
            mappings = self._parse_json_list_of_dicts(
                metadata["external_source_mappings_json"]
            )
            for mapping in mappings:
        ...
        for target_taxon_id in unique_taxon_ids:
            target_metadata = metadata_by_taxon_id.get(target_taxon_id)
            if target_metadata is None:
                continue
            target_candidates: list[dict[str, object]] = []
            hints = self._parse_json_list_of_dicts(
                target_metadata["external_similarity_hints_json"]
            )
            for hint in hints:
                if str(hint.get("source_name") or "") != "inaturalist":
                    continue
                source_taxon_id = str(hint.get("external_taxon_id") or "").strip()
                if not source_taxon_id:
                    continue
                mapped_taxon_id = inat_external_id_to_canonical_taxon_id.get(source_taxon_id)
                label = self._hint_label(hint)
                confidence = self._hint_confidence(hint)
                if mapped_taxon_id:
                    referenced = self._upsert_referenced_taxon(
                        connection,
                        source_taxon_id=source_taxon_id,
                        scientific_name=label
                        or self._canonical_taxon_label(metadata_by_taxon_id[mapped_taxon_id]),
                        preferred_common_name=str(hint.get("common_name") or "").strip() or None,
                        mapping_status=ReferencedTaxonMappingStatus.MAPPED,
                        mapped_canonical_taxon_id=mapped_taxon_id,
                        reason_codes=["inat_similar_species", "mapped"],
                    )
                    target_candidates.append(
                        {
                            "canonical_taxon_id": mapped_taxon_id,
                            "taxon_label": self._canonical_taxon_label(
                                metadata_by_taxon_id[mapped_taxon_id]
                            ),
                            "playable_item_id": None,
                            "source": "inaturalist.similar_species",
                            "score": (
                                0.55
                                if distractor_policy.prefer_inat_similar_species
                                else 0.35
                            ),
                            "reason_codes": ["inat_similar_species"],
                            "referenced_only": False,
                            "out_of_pack": mapped_taxon_id not in unique_taxon_ids,
                            "sort_key": (0, 0, 0, mapped_taxon_id, referenced.referenced_taxon_id),
                        }
                    )
                    continue

                if not label:
                    self._upsert_referenced_taxon(
                        connection,
                        source_taxon_id=source_taxon_id,
                        scientific_name=f"unknown:{source_taxon_id}",
                        preferred_common_name=None,
                        mapping_status=ReferencedTaxonMappingStatus.IGNORED,
                ...
                if confidence is not None and confidence < 0.5:
                    self._upsert_referenced_taxon(
                        ...
                        scientific_name=label,
                        preferred_common_name=str(hint.get("common_name") or "").strip() or None,
                        mapping_status=ReferencedTaxonMappingStatus.AUTO_REFERENCED_LOW_CONFIDENCE,
                        mapped_canonical_taxon_id=None,
                        reason_codes=["inat_similar_species", "low_confidence"],
                    )
                    continue
                ...
                referenced = self._upsert_referenced_taxon(
                    ...
                    scientific_name=label,
                    preferred_common_name=str(hint.get("common_name") or "").strip() or None,
                    mapping_status=ReferencedTaxonMappingStatus.AUTO_REFERENCED_HIGH_CONFIDENCE,
                    mapped_canonical_taxon_id=None,
                    reason_codes=["inat_similar_species", "auto_referenced_high_confidence"],
                )
                target_candidates.append(
                    {
                        "canonical_taxon_id": referenced.referenced_taxon_id,
                        "taxon_label": self._referenced_taxon_label(referenced),
                        "playable_item_id": None,
                        "source": "inaturalist.similar_species",
                        "score": 0.55 if distractor_policy.prefer_inat_similar_species else 0.35,
                        "reason_codes": ["inat_similar_species", "referenced_only"],
                        "referenced_only": True,
                        "out_of_pack": True,
                        "sort_key": (0, 0, 0, referenced.referenced_taxon_id, source_taxon_id),
                    }
                )
            candidates_by_target[target_taxon_id] = target_candidates
        return candidates_by_target

    def _fetch_canonical_taxon_metadata(
        self,
        connection: psycopg.Connection,
    ...
    def _upsert_referenced_taxon(
        ...
        *,
        ...
        mapped_canonical_taxon_id: str | None,
        reason_codes: list[str],
    ) -> ReferencedTaxon:
        now = datetime.now(UTC)
        referenced_taxon = ReferencedTaxon(
            ...
            common_names_i18n=(
                {"en": [preferred_common_name], "fr": [], "nl": []}
        ...
        payload = referenced_taxon.model_dump(mode="json")
        connection.execute(
            """
            ...
            ON CONFLICT (source, source_taxon_id)
            DO UPDATE SET
                ...
                mapped_canonical_taxon_id = EXCLUDED.mapped_canonical_taxon_id,
                reason_codes_json = EXCLUDED.reason_codes_json,
                updated_at = EXCLUDED.updated_at,
                payload_json = EXCLUDED.payload_json
            """,
            (
                referenced_taxon.referenced_taxon_id,
                referenced_taxon.source,
                referenced_taxon.source_taxon_id,
                referenced_taxon.scientific_name,
                referenced_taxon.preferred_common_name,
                _json(referenced_taxon.common_names_i18n),
                referenced_taxon.rank,
                referenced_taxon.taxon_group,
                referenced_taxon.mapping_status,
                referenced_taxon.mapped_canonical_taxon_id,
                _json(referenced_taxon.reason_codes),
                referenced_taxon.created_at.isoformat(),
                now.isoformat(),
                _json(payload),
            ),
        )
        connection.execute(
            """
            INSERT INTO referenced_taxon_events (
                referenced_taxon_event_id,
                referenced_taxon_id,
                source,
                source_taxon_id,
                mapping_status,
            ...
            """,
            (
                f"reftaxonevt:{referenced_taxon.referenced_taxon_id}:{uuid4().hex[:8]}",
                referenced_taxon.referenced_taxon_id,
                referenced_taxon.source,
                referenced_taxon.source_taxon_id,
                referenced_taxon.mapping_status,
    ...
    def _row_taxon_label(self, row: dict[str, object]) -> str:
        label = str(row.get("taxon_label") or "").strip()
        if label:
            return label
        common_names = self._parse_common_names_i18n(row.get("common_names_i18n_json"))
        for language in ("fr", "en", "nl"):
            names = common_names.get(language) or []
            if names:
                return names[0]
        scientific_name = str(row.get("scientific_name") or "").strip()
        if scientific_name:
            return scientific_name
        return str(row["canonical_taxon_id"])

    def _canonical_taxon_label(self, metadata: dict[str, object]) -> str:
        common_names = self._parse_json_string_list(metadata.get("common_names_json"))
        if common_names:
            return common_names[0]
        return str(metadata.get("accepted_scientific_name") or "").strip()

    def _referenced_taxon_label(self, referenced_taxon: ReferencedTaxon) -> str:
        if referenced_taxon.preferred_common_name:
            return referenced_taxon.preferred_common_name
        return referenced_taxon.scientific_name

    def _hint_label(self, hint: dict[str, object]) -> str:
        common_name = str(hint.get("common_name") or "").strip()
        if common_name:
            return common_name
        return str(hint.get("accepted_scientific_name") or "").strip()

    def _hint_confidence(self, hint: dict[str, object]) -> float | None:
        value = hint.get("confidence")
        if value is None:
            return None
        try:
            return float(value)
    ...
    def _parse_json_string_list(self, raw_value: object) -> list[str]:
        if isinstance(raw_value, str):
            try:
                parsed = json.loads(raw_value)
            except json.JSONDecodeError:
    ...
    def _parse_common_names_i18n(self, raw_value: object) -> dict[str, list[str]]:
        if isinstance(raw_value, str):
            try:
                parsed = json.loads(raw_value)
            except json.JSONDecodeError:
