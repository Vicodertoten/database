...
Contract note: this map predates the current contract cleanup. Mentions of
`pack.materialization.v2` are historical/semantic audit references only. Current
runtime contract status is defined in `docs/architecture/contract-map.md`.
...
4. Build/locate one real `pack.materialization.v2` only when maintaining the
   historical v2 semantic reference family.
